...
data class TaskListUIState(
    ...
    val taskType: ToolTag = ToolTag.GenericTask,
...
class TaskListViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow<TaskListUIState>(TaskListUIState(patientCode = patientCod))
    ...
    fun downloadData(){
        ...
        viewModelScope.launch {
            ...
            val tasks = agendaTaskRepository.getAgendaTasks(toolTag.getType(),dateParam,patientCod).results ?: listOf()
            val taskListItems = tasks.map { TaskListItem(
                name = it.entityName,
                time = it.execTime ?: "",
                executed = it.execTime != null,
                hasDataToUpload = false
            ) }
            _uiState.value = TaskListUIState(
                taskType = toolTag,
                patientName = patient?.name ?: "",
                selectedDate = "21/08/2023",
                selectedShift = null,
